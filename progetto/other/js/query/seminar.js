...
const getByEvent=(req,res)=>{
  db.select('seminar.idseminar','seminar.place','seminar.title','seminar.description','seminar.dateAndTime').from('seminar').join('artisticEvent',{'artisticEvent.idseminar':'seminar.idseminar'}).where('artisticEvent.idevent', req.params.id).orderBy('dateAndTime').then(function(data){
    res.json(data);
  })
}
